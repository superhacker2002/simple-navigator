...
public:
    // FUNCTIONS
    List(const allocator_type &Alloc = allocator_type()) : size_(0), head_(nullptr), allocator_(Alloc) {
        make_end();
    }
    ...
    List(List &&other) : size_(other.size_), head_(other.head_), end_(other.end_) {
        other.head_ = nullptr;
        other.end_ = nullptr;
        other.size_ = 0;
    }
