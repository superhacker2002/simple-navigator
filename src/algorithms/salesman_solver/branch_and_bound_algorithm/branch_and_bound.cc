...
void BranchAndBound::pathSearching_(const s21::GraphData::MatrixType& matrix, PathsList& curr_path, double curr_lower_bound) {
  if (matrix.GetRows() == 2) {
      logPath_(curr_path);
  ...
  pathSearching_(curr_matrix, new_path, curr_lower_bound);  // searching subset containing zero_edge 

  curr_matrix = new_matrix;
  curr_matrix(zero_edge.first, zero_edge.second) = kMaxDistance;
  pathSearching_(curr_matrix, curr_path, curr_lower_bound);  // searching subset not containing zero_edge 
}
...
BranchAndBound::Edge BranchAndBound::findBestZeroPath_(const s21::GraphData::MatrixType& matrix) {
  ...
  auto max_it = std::find(coefficients.begin(), coefficients.end(), max_coefficient);
  return zeroes[std::distance(coefficients.begin(), max_it)];
}

double BranchAndBound::getCoefficient_(const s21::GraphData::MatrixType& matrix, int from, int to) {
    double rmin, cmin;
